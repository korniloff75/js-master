...
/* 	function __construct()
	...
	*/
	...
	/*
	Универсальная функция получения информации по:
	$inf:
	balance
	free - Получить информацию о бесплатных сообщениях и его использовании
	limit - Получение текущего состояния вашего дневного лимита.
	senders - Получение списка отправителей
	...
	*/
	public function getInfo($inf, $post = null)
		{
		$pref= $inf==='cost'? '/sms/': '/my/';
		$post = $post ?? new stdClass();
		$post->api_id= $post->api_id ?? $this->ApiKey;
		$url=$this->protocol.'://'.$this->domain . $pref . $inf;
		$request=$this->Request($url, $post);
		return $this->CheckReplyError($request, 'get' . ucfirst($inf));
		}
