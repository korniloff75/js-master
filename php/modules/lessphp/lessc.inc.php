...
class lessc {
	...
	public function checkedCompile($in, $out) {
		/* var_dump(
			// $in, $out, realpath($out),
			!is_file($out) || filemtime($in) > filemtime($out)
		); */

		// echo $this->compileFile($in, $out);

		if (!is_file($out) || filemtime($in) > filemtime($out)) {
			$suc_comp = $this->compileFile($in, $out);

			if(!$suc_comp) {
				var_dump($in, $out, realpath($out));
			}
