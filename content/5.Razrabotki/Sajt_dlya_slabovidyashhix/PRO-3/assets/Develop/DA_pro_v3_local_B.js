...
window.DizSel = window.DizSel || !!_K && {
	...
	altObj: _K.G('#altObj') || _K.body().cr('div', { id: 'altObj', style: '/* position:absolute; */ left:0; top:0; z-index:30000; width:100%; padding:20px; margin:0;', hidden: 1 }),
	...
	PUopacity: function () {
		...
		DizSel.PUanimate = setInterval(function () {
			if (DizSel.PU.style.opacity <= 0.1) {
				return clearInterval(DizSel.PUanimate);
			}
			DizSel.PU.style.opacity -= .015;
			// console.log(DizSel.PU.style.opacity);
		}, 30);
	},
	...
	CheckCook: function () { //== При загрузке и КАЖДОМ КЛИКЕ на SlabovidButton
		...
		if (DizSel.v.DAlt) {
			...
			DizSel.altBody.className = Cook.get('diz_alt_class') || '';
	...
	fns: { //== Вложенные ФУНКЦИИ
		getNodes: function getNodesSelf(handler, els) { //== Универсальный метод рекурсивной обработки всех потомков заданных элементов
			els = (els || DizSel.altBody).childNodes;
		...
		toggleStyle: function (stName, e) {
			...
			//			if(stName)
			DizSel.altBody.classList.toggle(stName);
			Cook.set({ diz_alt_class: DizSel.altBody.className }, DizSel.sts.mem);
		},

		imageOff: function () {
			for (var i = 0, imgs = DizSel.altBody.G('A$img'), l = imgs.length; i < l; i++) {
		...
		sound: function () {
			...
			DizSel.altBody.onmouseup = function (e) {
				e = _K.Event.fix(e);
	...
	createBGs: function () { //== Запускается ПРИ ЗАГРУЗКЕ
		...
		DizSel.PU.e.add('select', function (e) { _K.e.fix(e).preventDefault() });
