...
window.DizSel = window.DizSel || !!_K && {
	...
	Backup: {
		...
		save: function () {
			...
			[].forEach.call(_K.G('A$div#altObj .DA_del'), function (i) { _K.G(i).del() });
	...
	init: function () {
		...
		_K.l("DizSel.inited= " + DizSel.inited);
		if (!_K.isObject()) {
			console.error('missing _K');
			return;
		}
		if (!_K.prot || !!_K.v.diz) return;
		DizSel.linki = _K.G('A$body link'); 
	...
	CheckCook: function () { //== При загрузке и КАЖДОМ КЛИКЕ на SlabovidButton
		...
		} else _K.G(DizSel.Backup.altObj).style.top = (parseInt(getComputedStyle(DizSel.PU).height) || 45) + 'px';
		DizSel.log.push('getComputedStyle(DizSel.PU).height= ' + getComputedStyle(DizSel.PU).height);
		DizSel.PU.e.add({
			mouseover: 'clearInterval (DizSel.PUanimate); this.style.opacity=1;',
		...
		if (DizSel.v.DAlt) {
			...
			clearInterval (DizSel.PUanimate);
	...
	fns: { //== Вложенные ФУНКЦИИ	
		...
		toggleStyle: function (stName, e) {
			_K.l("arguments[0]= " + arguments[0]);
			_K.l("arguments[1]= " + arguments[1]);
			_K.l("arguments[2]= " + arguments[2]);
			//			if(stName)
...
// DA_del
_K.G('#widget-e1159542-5367-c83e-40da-dd2af1610533').classList.add('DA_del');
// zIndex
_K.DR(function() {
	[].forEach.call(_K.G('A$#altObj #navigation-23820a2c-c889-4f1f-9015-9a77fb783960 li ul'), function(i) { _K.G(i, {zIndex: '10000', position:'static'})});
})
