...
window.DizSel = window.DizSel || !!_K && {
	...
	set checkUrl(uri) { Object.defineProperty(this, 'checkUrl', { get: function () { return uri } }); },
	...
	Backup: {
		...
		altBody: _K.G(this.altBody) || this.altObj.cr('div', { id: 'altBody' }),
		...
		save: function () {
			...
			_K.G('A$div#altObj .DA_del').forEach(function (i) { _K.G(i).del() });
			DizSel.fns.getNodes(function (i) {
				DizSel.fns.setCol(i);
				DizSel.fns.setFS(i)
	...
	CheckCook: function () { //== При загрузке и КАЖДОМ КЛИКЕ на SlabovidButton
		...
		} else _K.G(DizSel.Backup.altObj).style.paddingTop = (parseInt(getComputedStyle(DizSel.PU).height) || 45) + 'px';
...
// DA_del
_K.G('#widget-0ba4e0c7-6c8a-23c8-27e2-0ba95251756f').classList.add('DA_del');
// zIndex
_K.DR(function() {
