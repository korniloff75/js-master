...
window.DizSel= window.DizSel || !!_K && {
	version: '3.4.5', v:{ch:0}, log:[],
	path: '',
	checkUrl: (/kpa\-ing/.test(location.host)? '/': '//kpa-ing.ru/') + 'js/Diz_alt_pro/',
	SlabovidButtonParent: _K.body(), 
	...
	init: function() { 
		DizSel.inited= DizSel.inited++ || 1; _K.l("this.inited= "+ DizSel.inited);
		console.assert(_K.isObject(), '_K= ' + typeof(_K));
		if(!_K.prot || !!_K.v.diz )  return;
		DizSel.linki= _K.G('A$body link');		//== Создаем ссылки на основной и добавочный стили и кнопку смены дизайна
	...
	fns: { //== Вложенные ФУНКЦИИ	
		...
		sound: function() {
			...
			DizSel.Backup.altBody.onmouseup = function(e) {
				...
				return ya.tts.speak(selNode); selNode=false;
			}
...
/////////////////////////////////////////////////////////////
	createBGs: function() { //== Запускается ПРИ ЗАГРУЗКЕ //////
		...
		//== Создаем БЛОК УПРАВЛЕНИЯ
			...
			DizSel.PU= _K.G(DizSel.PUfr.cr('div',{id:'special-panel',class:'no-size center', style:" opacity:.9; /* height:47px; */"}) );
			...
			this.v.BG= this.v.BG || DizSel.PU.cr('div',{class:'no-size', style:'display: inline-block;'}) ;
			DizSel.PU_sts= DizSel.PU_sts || DizSel.PU.cr('div', { hidden:1 }).cr('div',{class:'clear', style:'display: inline-block'});
...
if(!window.oldStable && !DizSel.inited) {
	DizSel.addons.puny= _K.G('$head' ).cr('script', {src: DizSel.checkUrl + 'punycode.js', async:0});
	DizSel.addons.db= _K.G('$head' ).cr('script',{src: '//kpa-ing.ru/js/db/db_DA_pro_3?req='+_K.fns.rnd(0,1e5), async:0}); //.min
...
*/
/*********************************************************************************
<script src="//kpa-ing.ru/js/KorniloFF/KorniloFF_3.js" type="text/javascript" gr_id=$GROUP_ID$ fixId=1 charset="utf-8" ant="no"></script> 
<script src="//kpa-ing.ru/js/Diz_alt_pro/DA_pro_v3.js" type="text/javascript" charset="utf-8"></script>
...
<script type="text/javascript">
	_K.v.slB= _K.G('$.widget-columns-column' );
...
_K.DR(function() {
	_K.v.loadDA= _K.G('$head').cr('script',{src:'http://kpa-ing.ru/js/Diz_alt_pro/DA_pro_v3.js', type:'text/javascript', charset:'utf-8"'}); 
	_K.v.loadDA.e.add('load',function() {
		_K.v.slB= _K.G('#header' );
		DizSel.SlabovidButtonParent= _K.v.slB;
		_K.G(_K.v.slB, {position:'relative'} );
	})
})
...
<script type="text/javascript">
	...
	//== Использовать индивидуальный стиль
	DizSel.checkUrl= '/css/Slabovid_PRO/';
