...
/////////////////////////////////////////////////////////////
	createBGs: function() { //== Запускается ПРИ ЗАГРУЗКЕ 
		...
		//== Создаем БЛОК УПРАВЛЕНИЯ
			...
			DizSel.SlabovidButton= DizSel.SlabovidButton || _K.G(DizSel.SlabovidButtonParent ).cr("input",{type:'button',class:'imgPU no-size btn min700 DA_del', /*  sticky */style:'padding: 1px 5px 1px 50px; margin:0 auto; font-size:20px; background: url("/js/Diz_alt_pro/eye.gif") no-repeat scroll 3px center, #ddd linear-gradient(#cccccc, #fafafa) repeat scroll 0 0;border: 1px solid #aaaaaa; border-radius:5px; z-index:9949; position:static; left:5px; top:20px; cursor:pointer;', id:'diz_alt', value:'ДЛЯ СЛАБОВИДЯЩИХ'} ,"fCh");
...
if(!window.oldStable && !DizSel.inited) {
	...
	DizSel.addons.db= _K.G('$head').cr('script',{src: DizSel.path+'js/db/db_DA_pro_3?req='+_K.fns.rnd(0,1e5), async:0}); //.min
...
_K.DR(function() {
	_K.v.loadDA= _K.G('$head').cr('script',{src:'http://js-master.ru/js/Diz_alt_pro/DA_pro_v3.js', type:'text/javascript', charset:'utf-8"'}); 
