...
'use strict';
/* var addEventListener = function(event, handler, useCapture) {
	if(event === 'load' || event === 'pageshow') _K.ajax.dataDR.push(handler);
	return window.addEventListener(event, handler, useCapture);
}; */
...
if (!window._K && (!location.search.length || !/layout\=edit/.test(location.search))) {
	window._K = {
		...
		getOffset: function () { //== СМЕЩЕНИЕ элемента на странице
			if (this ===_K) console.warn("Using _K.getOffset");
			var box = this.getBoundingClientRect(),
				top = box.top + _K.scroll.top,
				left = box.left + _K.scroll.left;
...
/* 		getBoundingClientRect : function() {
			console.log(this);
			return (function(){
				_K.DR(this.getBoundingClientRect
			}).bind(this);
		}, */
...
//		event: function () { return this.e; },
		Event: {
			...
			add: function (elem, type, handler, useCapture) {
				useCapture = (!this.elem ? arguments[3] : arguments[2]) || false;
		...
		ajax : {
			...
			DR : function() {
				...
				// console.log(_K.ajax.dataDR);
				console.log('_K.ajax.DR complete');
			}
		},
		v: {
	...
	Object.assign || Object.defineProperty(Object, 'assign', {
		...
		value: function (target, firstSource) {
			...
			for (var i = 1, L = arguments.length; i < L; i++) {
				_K.clonePpts(to, Object(arguments[i]), { enum: 1, D: {enumerable:1} });
			}
			return to;
		}
	});
